  ...
  const displayImage = imageId => {
    ...
    const currentImg = async i => {
      try {
        ...
        const { url } = imageJson;
        setActiveImg(activeImg !== url ? url : '');
      } catch (err) {
        console.error(err);
      }
    };
...
function ImageList({ images, onClick }) {
  if (images.length === 0)
    return (
      <p>
        Oops no photos, take a selfie at the camera{' '}
        <a href="/camera" className="link-secondary">
          page
        </a>
        !
      </p>
  ...
  const imagesList = images.map(images => (
    <li
      ...
      key={images.imageId}
      onClick={() => onClick(images.imageId)}>
